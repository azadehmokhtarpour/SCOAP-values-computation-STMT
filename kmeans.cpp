...
//these codes puts the evaluated <CC,CO> in 2 clusters based on kmeans algorithm
//except function "Evaluate_Kmeans" all others are from "https://www.medphysics.wisc.edu/~ethan/kmeans/" 
...
void Evaluate_Kmeans(struct parse_state *state) {
	double centroid[3][2];
	//double cc_co[MAX_INPUT + MAX_OUTPUT + MAX_WIRES][2];
	//double sc_so[MAX_INPUT + MAX_OUTPUT + MAX_WIRES][2];
	//int o[MAX_INPUT + MAX_OUTPUT + MAX_WIRES];
	...
	for (int iM = 0; iM < state->nModule; iM++)
	{
		...
		int nTotal = m->nIn + m->nOut + m->nWires;
		double *cc_co = new double[2 * nTotal];
		double *sc_so = new double[2 * nTotal];
		int *o = new int[nTotal];
		printf("debugging kmeans:\n");
		printf("hmodule: %s\n", m->name);
		
		for (int i = 0; i < m->nIn+m->nOut+m->nWires; i++)
		{
			if (i < m->nIn) {
				//cc_co[i][0] = m->p_inputs[i]->st.s.total_cc;
				cc_co[2*i] = m->p_inputs[i]->st.s.total_cc;
				//cc_co[i][1] = m->p_inputs[i]->st.s.co;
				cc_co[2*i+1] = m->p_inputs[i]->st.s.co;
				//sc_so[i][0] = m->p_inputs[i]->st.s.total_sc;
				sc_so[2*i] = m->p_inputs[i]->st.s.total_sc;
				//sc_so[i][1] = m->p_inputs[i]->st.s.so;
				sc_so[2*i+1] = m->p_inputs[i]->st.s.so;
			}
			else if (i < m->nIn + m->nOut) {
				//cc_co[i][0] = m->p_outputs[i - m->nIn]->st.s.total_cc;
				//cc_co[i][1] = m->p_outputs[i - m->nIn]->st.s.co;
				//sc_so[i][0] = m->p_outputs[i - m->nIn]->st.s.total_sc;
				//sc_so[i][1] = m->p_outputs[i - m->nIn]->st.s.so;
				cc_co[2*i + 0] = m->p_outputs[i - m->nIn]->st.s.total_cc;
				cc_co[2*i + 1] = m->p_outputs[i - m->nIn]->st.s.co;
				sc_so[2*i + 0] = m->p_outputs[i - m->nIn]->st.s.total_sc;
				sc_so[2*i + 1] = m->p_outputs[i - m->nIn]->st.s.so;
			}
			else {
				//cc_co[i][0] = m->p_wires[i - (m->nIn + m->nOut)]->st.s.total_cc;
				//cc_co[i][1] = m->p_wires[i - (m->nIn + m->nOut)]->st.s.co;
				//sc_so[i][0] = m->p_wires[i - (m->nIn + m->nOut)]->st.s.total_sc;
				//sc_so[i][1] = m->p_wires[i - (m->nIn + m->nOut)]->st.s.so;
				cc_co[2*i  ] = m->p_wires[i - (m->nIn + m->nOut)]->st.s.total_cc;
				cc_co[2*i+1] = m->p_wires[i - (m->nIn + m->nOut)]->st.s.co;
				sc_so[2*i+0] = m->p_wires[i - (m->nIn + m->nOut)]->st.s.total_sc;
				sc_so[2*i+1] = m->p_wires[i - (m->nIn + m->nOut)]->st.s.so;
			}
		}
		srand((unsigned)time(&t));
		random = rand() % m->nIn + m->nOut + m->nWires;
		step = rand() % m->nIn + m->nOut + m->nWires;
		for (int j = 0; j < k; j++)
		{
			printf("random%d: %d\n", j, random);
			centroid[j][0] = cc_co[2*random];
			centroid[j][1] = cc_co[2*random+1];
			random = (random + step) % (m->nIn + m->nOut + m->nWires);
		}
		printf("cluster0: %f,%f\ncluster1: %f,%f\ncluster2: %f,%f\n",
			centroid[0][0], centroid[0][1], centroid[1][0], centroid[1][1], centroid[2][0], centroid[2][1]);
		//test the correct centroid
		centroid[0][0] = 0; centroid[0][1] = 0;
		centroid[1][0] = 0; centroid[1][1] = INF;
		centroid[2][0] = INF; centroid[2][1] = 0;
//		centroid[3][0] = INF; centroid[3][1] = INF;
		printf("CC kmeans\n");
		kmeans(2, cc_co, m->nIn + m->nOut + m->nWires, k, *centroid, o);
		printf("SC kmeans\n");
		kmeans(2, sc_so, m->nIn + m->nOut + m->nWires, k, *centroid, o);
